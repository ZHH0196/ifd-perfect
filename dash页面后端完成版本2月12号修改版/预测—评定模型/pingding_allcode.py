...
# 设置纵轴范围为 -50 到 50
plt.ylim([-60, 60])
# 设置横轴范围为 0 到 2803 个样本（假设每个样本对应一个文件）
plt.xlim([0, 3000 * 2560])
# 设置横轴的刻度为每 500 个样本标注一次
plt.xticks(range(0, len(all_signals) * 2560, 500 * 2560), range(0, 3000, 500))
# 取消网格线
plt.grid(True)
...
plt.xlim([0, 3000])
# 设置横轴的刻度为每 500 个样本标注一次
plt.xticks(range(0, len(rms_values), 500), range(0, 3000, 500))
# 取消网格线
plt.grid(True)
# 保存图形为PNG文件
...
plt.xlim([0, 3000])
# 设置横轴的刻度为每 500 个样本标注一次
plt.xticks(range(0, len(rmv_values), 500), range(0, 3000, 500))
# 取消网格线
plt.grid(True)
# 保存图形为PNG文件
...
plt.xlim([0, 3000])
# 设置横轴的刻度为每 500 个样本标注一次
plt.xticks(range(0, len(sigmoid_values), 500), range(0, 3000, 500))
# 取消网格线
plt.grid(True)
# 保存图形为PNG文件
...
plt.ylabel('指标值')
plt.xlim([0,3000])
plt.xticks(range(0, len(ts_rank_values), 500), range(0, 3000, 500))
plt.grid(True)
...
plt.ylabel('CDI指标值')
plt.xlim([0,3000])
plt.xticks(range(0, len(CDI), 500), range(0, 3000, 500))
plt.grid(True)
...
cdi_data = np.array(CDI)

# 时间序列
time_series = np.arange(len(cdi_data))
...
x0 = np.array([cdi_data[0]])  # 初始状态 (使用CDI的第一个值作为初始状态)

# 创建卡尔曼滤波器
kf = AdaptiveKalmanFilter(A, B, C, Q, R, P, x0)

# =========================
# Step 4: 预测轴承剩余使用寿命 (RUL)
# =========================
...
# 绘制CDI数据与RUL预测结果
plt.figure(figsize=(10, 6))
plt.plot(time_series, cdi_data, label='CDI Data')
plt.plot(time_series, rul_predictions, label='RUL Prediction')
plt.xlabel('Time')
plt.ylabel('CDI / RUL')
plt.title('CDI vs RUL Prediction')
plt.legend()
plt.grid(True)
plt.close()

# 输出最后时刻的RUL预测值
print(f"轴承失效阈值为: {rul_predictions[-1]}")
