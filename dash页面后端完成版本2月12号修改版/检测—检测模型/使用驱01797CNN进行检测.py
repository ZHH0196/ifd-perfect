...
new_data_df = pd.read_csv('驱动端外圈右边0.021断层.csv')

# 设置参数
samples_per_block = 1681  
model_path = "12K0马力1797电机速驱动端.h5"  # 模型

# 训练时的状态标签列表，按顺序列出
state_labels = ['驱动端内圈0.007断层', '驱动端内圈0.014断层', '驱动端内圈0.021断层', '驱动端内圈0.028断层', '驱动端外圈中心0.007断层', '驱动端外圈中心0.014断层', '驱动端外圈中心0.021断层', '驱动端外圈右边0.007断层', '驱动端外圈右边0.021断层', '驱动端外圈左边0.007断层', '驱动端外圈左边0.021断层', '驱动端正常状态', '驱动端球体0.007断层', '驱动端球体0.014断层', '驱动端球体0.021断层', '驱动端球体0.028断层']
# 对新数据进行故障检测
