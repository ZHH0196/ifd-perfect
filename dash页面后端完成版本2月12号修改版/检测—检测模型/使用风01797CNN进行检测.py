...
import matplotlib
# 设置全局中文字体为黑体（同样可按需替换字体及路径）
matplotlib.rcParams['font.sans-serif'] = ['SimHei']
# 解决负号显示问题（如果图表中可能出现负号的话）
...
def DetectNewDataFromDataFrame(new_data_df, model_path, samples_per_block, state_labels):
    ...
    for i in range(len(state_labels)):
        print(f'{state_labels[i]}的概率为:{str(predictions[0][i]*100)[:5]}%')
    # 打印检测到的故障状态标签
    print(f"检测的故障状态标签: {state_labels[predicted_label_idx]}")


new_data_df = pd.read_csv('风扇端内圈0.014断层.csv')


# 设置参数
samples_per_block = 1681  
model_path = "12K0马力1797电机速风扇端.h5"  # 模型

# 训练时的状态标签列表，按顺序列出
state_labels = ['风扇端内圈0.007断层', '风扇端内圈0.014断层', '风扇端内圈0.021断层', '风扇端外圈中心0.007断层', '风扇端外圈中心0.014断层', '风扇端外圈右边0.007断层', '风扇端外圈左边0.007断层', '风扇端外圈左边0.014断层', '风扇端外圈左边0.021断层', '风扇端正常状态', '风扇端球体0.007断层', '风扇端球体0.014断层', '风扇端球体0.021断层']
